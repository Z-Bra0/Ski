...
func TestAddSelectedUsesManifestTargetAlias(t *testing.T) {
	t.Parallel()
	...
	added, warnings, err := svc.AddSelected("git:"+repo.URL, nil, "", false, nil)
	if err != nil {
		t.Fatalf("AddSelected() error = %v", err)
	}
	if got, want := added, []string{"repo-map"}; !sameStrings(got, want) {
	...
	if got, want := doc.TargetAlias["cc"], "dir:./agent/cc/skills"; got != want {
		t.Fatalf("manifest target_alias cc = %q, want %q", got, want)
	}
	...
	if got, want := lf.Skills[0].Targets, []string{"cc"}; !sameStrings(got, want) {
		t.Fatalf("lockfile targets = %#v, want %#v", got, want)
	}

	storePath := filepath.Join(homeDir, ".ski", "store", "git", "repo-map", repo.Commit)
	aliasLink := filepath.Join(projectDir, "agent", "cc", "skills", "repo-map")
	targetPath, err := os.Readlink(aliasLink)
	if err != nil {
		t.Fatalf("Readlink(alias target) error = %v", err)
	}
	if targetPath != storePath {
		t.Fatalf("alias symlink target = %q, want %q", targetPath, storePath)
	}
}

func TestRemoveUsesTargetAliasOverride(t *testing.T) {
	t.Parallel()

	projectDir := t.TempDir()
	homeDir := t.TempDir()
	manifestPath := filepath.Join(projectDir, manifest.FileName)
	storePath := filepath.Join(homeDir, ".ski", "store", "git", "repo-map", "abc123")
	...
	if err := lockfile.WriteFile(lockfile.Path(projectDir), lockfile.Lockfile{
		Version: 1,
	...
	if err := target.Link(projectDir, "dir:./agent/cc/skills", "repo-map", storePath); err != nil {
		t.Fatalf("Link(alias target) error = %v", err)
	}
	...
	if err := svc.Remove("repo-map", []string{"cc"}); err != nil {
		t.Fatalf("Remove() error = %v", err)
	}

	docAfter, err := manifest.ReadFile(manifestPath)
	if err != nil {
		t.Fatalf("ReadFile(manifest after remove) error = %v", err)
	}
	if len(docAfter.Skills) != 0 {
		t.Fatalf("manifest skills = %#v, want empty after alias remove", docAfter.Skills)
	}

	if _, err := os.Lstat(filepath.Join(projectDir, "agent", "cc", "skills", "repo-map")); !os.IsNotExist(err) {
		t.Fatalf("alias link stat error = %v, want not exist", err)
	}
}
...
func TestListRejectsTargetAliasBuiltInConflict(t *testing.T) {
	t.Parallel()
	...
	if err := manifest.WriteFile(manifestPath, manifest.Manifest{
		Version:     1,
		Targets:     []string{"claude"},
		TargetAlias: map[string]string{"claude": "dir:./agent/cc/skills"},
	}); err != nil {
		t.Fatalf("WriteFile(manifest) error = %v", err)
	}
	...
	_, err := svc.List()
	if err == nil {
		t.Fatal("List() error = nil, want target alias conflict error")
	}
	if !strings.Contains(err.Error(), `target aliases: alias "claude" conflicts with a built-in target`) {
		t.Fatalf("List() error = %v, want alias conflict error", err)
	}
}
